...
def calculate_masses(layer2, beta, bias, y):
    ...
    for i in phi:
        ...
        m1 = eta * eta_neg * eta_plus * np.exp(-w_neg[0]) * (
                    np.exp(w_plus[0]) - 1 + ((1 - np.exp(-w_neg[1])) * (1 - np.exp(-w_neg[2]))))
        m2 = eta * eta_neg * eta_plus * np.exp(-w_neg[1]) * (
                    np.exp(w_plus[1]) - 1 + ((1 - np.exp(-w_neg[0])) * (1 - np.exp(-w_neg[2]))))
        m3 = eta * eta_neg * eta_plus * np.exp(-w_neg[2]) * (
                    np.exp(w_plus[2]) - 1 + ((1 - np.exp(-w_neg[0])) * (1 - np.exp(-w_neg[1]))))
...
def join_mass_functions(mass_function_1, mass_function_2, mass_function_3, mass_function_12, mass_function_13,
                        mass_function_23, mass_function_123):
    mass_functions = []

    for t1, t2, t3, t12, t13, t23, t123 in zip(mass_function_1, mass_function_2, mass_function_3, mass_function_12,
                                               mass_function_13, mass_function_23, mass_function_123):
...
def calculate_ca_rule(t1_plausibility_array, t2_plausibility_array, t3_plausibility_array, t1_belief_array,
                      t2_belief_array, t3_belief_array):
    decision_ca = []
    ...
    for pl_theta1, pl_theta2, pl_theta3, bel_theta1, bel_theta2, bel_theta3 in zip(t1_plausibility_array,
                                                                                   t2_plausibility_array,
                                                                                   t3_plausibility_array,
                                                                                   t1_belief_array, t2_belief_array,
                                                                                   t3_belief_array):
...
def calculate_me_rule(t1_plausibility_array, t2_plausibility_array, t3_plausibility_array, t1_belief_array,
                      t2_belief_array, t3_belief_array):
    decision_me = []
...
def test_perturbed(X_test, y_test, classifier):
    ...
    mass_list, mass_function_1, mass_function_2, mass_function_3, mass_function_12, mass_function_13, mass_function_23, mass_function_123, conflict, eta, eta_plus, eta_neg = calculate_masses(
        layer2, beta, bias, y_test)
    ...
    mass_functions = join_mass_functions(mass_function_1, mass_function_2, mass_function_3, mass_function_12,
                                         mass_function_13, mass_function_23, mass_function_123)
    print("Done!")
    ...
    decision_ca = calculate_ca_rule(t1_plausibility_array, t2_plausibility_array, t3_plausibility_array,
                                    t1_belief_array, t2_belief_array, t3_belief_array)
    print("Done!")

    print("Calculating Maximal Element Rule... ", end="", flush=True)
    decision_me = calculate_me_rule(t1_plausibility_array, t2_plausibility_array, t3_plausibility_array,
                                    t1_belief_array, t2_belief_array, t3_belief_array)
    print("Done!")
    ...
    c1p1_count, c1p2_count, c1p3_count, c1p123_count, c2p1_count, c2p2_count, c2p3_count, c2p123_count, c3p1_count, c3p2_count, c3p3_count, c3p123_count = tabulate_id_rule(
        decision_id, y_test.cpu().detach().numpy())
